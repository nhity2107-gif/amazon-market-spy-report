...
`product_detail.html` is an index of product detail pages, and `product_detail\<asin>.html` shows image, title, seller, display-rank timeline, BSR timeline, source history, first seen, days tracked, Winner Signal Score, Amazon link, and the Winner Journey: First Seen -> New Release -> Seller Top10 -> Best Seller -> Stable Winner. Dashboard V2 surfaces `bought_past_month` as `Bought/mo` in Product Explorer rows and the Evidence Inspector, and on Competitor Explorer seller product tiles when the metric is available.
